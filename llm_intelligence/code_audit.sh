#!/bin/bash
# Code audit script for LLM Intelligence service
...
print_summary() {
  ...
  [ -n "$pylint_score" ] && echo "PYLINT: $pylint_score" || echo "PYLINT: (score not found)"
}

echo "=== RUFF CHECK ===" > audit_report.txt
uv run --group dev ruff check . >> audit_report.txt 2>&1

echo -e "\n=== BANDIT SECURITY ===" >> audit_report.txt
uv run --group dev bandit -r src >> audit_report.txt 2>&1

echo -e "\n=== MYPY TYPE CHECK ===" >> audit_report.txt
uv run --group dev mypy src >> audit_report.txt 2>&1

echo -e "\n=== RADON COMPLEXITY ===" >> audit_report.txt
uv run --group dev radon cc src -a >> audit_report.txt 2>&1

echo -e "\n=== VULTURE DEAD CODE ===" >> audit_report.txt
uv run --group dev vulture src >> audit_report.txt 2>&1

echo -e "\n=== PYLINT ===" >> audit_report.txt
uv run --group dev pylint src >> audit_report.txt 2>&1
echo -e "\n=== END ===" >> audit_report.txt

cat audit_report.txt | pbcopy 2>/dev/null || true
print_summary
echo "Done! Full report saved to 'audit_report.txt'."
