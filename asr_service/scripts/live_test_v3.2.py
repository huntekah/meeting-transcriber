#!/usr/bin/env python3
"""
Live Streaming Transcription Test (Qwen3-ASR-0.6B Version)

Real-time microphone transcription with VAD-based sentence boundary detection.
Uses Qwen3-ASR-0.6B for transcription (smaller, faster model).
...
def load_qwen_model(model_name: str = "Qwen/Qwen3-ASR-0.6B"):
    """
    Load Qwen3-ASR model using transformers backend.

    Args:
        model_name: Model name (Qwen/Qwen3-ASR-0.6B for speed)
    ...
    with Timer(f"Loading Qwen3-ASR ({model_name})"):
        model = Qwen3ASRModel.from_pretrained(
            ...
            max_inference_batch_size=64,  # Higher batch size for smaller model
            max_new_tokens=256,
        )
    return model
...
def main():
    ...
    print("\n" + "="*60)
    print("🎤 Live Streaming Transcription Test (Qwen3-ASR-0.6B)")
    print("="*60 + "\n")
    ...
    qwen_model_name = getattr(config, 'QWEN3_ASR_0_6B_MODEL', 'Qwen/Qwen3-ASR-0.6B')
    ...
    # Print statistics
    print("\n" + "="*60)
    print("📊 Session Statistics (Qwen3-ASR-0.6B)")
    print("="*60)
