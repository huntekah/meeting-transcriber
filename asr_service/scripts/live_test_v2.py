#!/usr/bin/env python3
"""
Live Streaming Transcription Test (MLX-Whisper Version)

Real-time microphone transcription with VAD-based sentence boundary detection.
Uses MLX-Whisper for native M4 Metal acceleration.
...
def load_whisper_model(model_name: str = "mlx-community/whisper-large-v3-turbo"):
    """
    Load Whisper model using MLX-Whisper (native Metal acceleration).

    Args:
        model_name: Model size (mlx-community/whisper-large-v3-turbo recommended)

    Returns:
        Loaded model path (MLX doesn't return model object)
    """
    with Timer(f"Loading MLX-Whisper ({model_name})"):
        # MLX-whisper downloads models automatically
        # Just return the model name - mlx_whisper.transcribe will load it
        pass
    return model_name
...
def consumer_thread(whisper_model_name: str, state: dict, stop_event: threading.Event):
    """
    ...
    Args:
        whisper_model_name: Model name/path for MLX-Whisper
    ...
    def transcribe(audio_np: np.ndarray, provisional: bool = False) -> str:
        ...
        # Suppress MLX-Whisper's progress bars by redirecting stdout/stderr
        with contextlib.redirect_stdout(open(os.devnull, 'w')), \
             contextlib.redirect_stderr(open(os.devnull, 'w')):
            # MLX-Whisper API with anti-hallucination settings
            result = mlx_whisper.transcribe(
                audio_np,
                path_or_hf_repo=whisper_model_name,
                language="en",
                # NOTE: Removed initial_prompt - causes severe loops in MLX
                # initial_prompt=state['previous_context'] if state['previous_context'] else None,
                condition_on_previous_text=False,  # Hallucination prevention
                word_timestamps=False,
                verbose=False,
                # Anti-hallucination parameters
                temperature=0.0,  # Use only greedy decoding
        ...
        # MLX-Whisper returns dict with 'segments' key
...
def main():
    ...
    print("\n" + "="*60)
    print("🎤 Live Streaming Transcription Test (MLX-Whisper)")
    print("="*60 + "\n")
    ...
    whisper_model_name = load_whisper_model("mlx-community/whisper-large-v3-turbo")
    ...
    consumer = threading.Thread(
        ...
        args=(whisper_model_name, state, stop_event),
        daemon=True
    )
    ...
    # Print statistics
    print("\n" + "="*60)
    print("📊 Session Statistics (mlx-whisper)")
    print("="*60)
